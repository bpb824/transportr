...
We can obtain travel times from the Google Directions API using `transportr`'s `travel_time()` function. There are several parameters you'll need to prepare before using the function:
...
```{r}
gKey = "AIzaSyAvxp8VHw_gJ7bzV7Xn7OkULBJZ3uWF7hs" #Bryan's Google API Key; get your own @ <>
nn_pdx = geocode_place("Nelson Nygaard Portland")
hotd = geocode_place("Hair of the Dog Portland")
...
```{r}
produce_row = geocode_place("Produce Row Portland")
...
```{r}
st_johns = geocode_place("St Johns Portland Oregon")
...
```{r}
falls = geocode_place("Multnomah Falls Oregon")
