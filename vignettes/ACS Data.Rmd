...
American Communtiy Survey data are used in a wide variety of applications by transportation planners. Until recently, much of this data required a fairly arduous querying, downloading, and cleaning process; the US Census Bureau's APIs and developers' query wrappers have made this task much easier. 
...
tractShape = rbind.SpatialPolygonsDataFrame(orShape,waShape)
...
subShape = tractShape[tractShape$TRACTCE %in% sub$tract &
                        tractShape$COUNTYFP %in% sub$county &
                        tractShape$STATEFP %in% sub$state,]
...
#Draw the polygons on the map using our palette and creating a popup with the bicycle commute share
  addPolygons(fillColor = ~pal(bike_prop),fillOpacity=0.5,weight=1.5,color=~pal(bike_prop),popup = ~htmlEscape(paste0(round(bike_prop*100,1),"%"))) %>%
